...
public class UpdateDownloadReceiver extends BroadcastReceiver {
    @Override public void onReceive(Context context,Intent intent){
        ...
        DownloadManager dm=(DownloadManager)context.getSystemService(Context.DOWNLOAD_SERVICE);if(dm==null)return;
        try(Cursor c=dm.query(new DownloadManager.Query().setFilterById(completed))){if(c==null||!c.moveToFirst()||c.getInt(c.getColumnIndexOrThrow(DownloadManager.COLUMN_STATUS))!=DownloadManager.STATUS_SUCCESSFUL){Toast.makeText(context,"Update download failed • Settings se retry karein",Toast.LENGTH_LONG).show();return;}}
        String name=prefs.getString("app_update_file","");File apk=new File(context.getExternalFilesDir(android.os.Environment.DIRECTORY_DOWNLOADS),name);
        ...
        try{context.startActivity(install);prefs.edit().remove("app_update_download_id").apply();}catch(Exception e){Toast.makeText(context,"Download notification open karke APK install karein",Toast.LENGTH_LONG).show();}
    }
}
