...
Then open **Actions → Build Signed Play AAB → Run workflow**. When the job is green, download the `LATHAEPS-SMART-Play-AAB-v3.23.36` artifact and unzip it. Upload the `.aab` file to Play Console.

## Create the signed release APK for direct download or Uptodown

Use the same four permanent signing secrets listed above. Open **Actions → Build Signed Release APK → Run workflow**. When the job is green, download and unzip the `LATHAEPS-SMART-Signed-Release-APK-v3.23.36` artifact. Share or upload `LATHAEPS_SMART_v3.23.36_RELEASE_SIGNED.apk`.
...
4. Upload `LATHAEPS_SMART_v3.23.36_Play_Signed.aab`.
