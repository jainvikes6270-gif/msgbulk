-- LATHAEPS SMART v3.23.36 secure subscription setup
...
create table if not exists public.app_subscriptions (
    ...
    plan text not null default 'trial' check (plan in ('trial','yearly','custom','lifetime','blocked')),
...
-- Safe upgrade for databases created by an older app version.
alter table public.app_subscriptions drop constraint if exists app_subscriptions_plan_check;
alter table public.app_subscriptions add constraint app_subscriptions_plan_check check (plan in ('trial','yearly','custom','lifetime','blocked'));
...
begin
    ...
    update public.app_subscriptions
       set plan='custom',enabled=true,expires_at=v_until,updated_at=clock_timestamp()
